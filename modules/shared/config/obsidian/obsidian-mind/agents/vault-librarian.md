...
1. **Orphan Detection**: Run `obsidian orphans` (or `grep -rL '\[\[' AI/work/ AI/org/ AI/perf/ AI/brain/ AI/reference/` as fallback). List notes with zero incoming links. For each, suggest which existing notes should link to them.
...
3. **Frontmatter Validation**: Glob all `.md` files in `AI/work/`, `AI/org/`, `AI/perf/`, `AI/brain/`, `AI/reference/`. Check each has:
   - `tags` (non-empty)
   - `date`
   - `description` (~150 chars)
   - Type-specific required fields (incidents need `ticket`, `severity`, `role`; work notes in recent quarters need `quarter`)

4. **Stale Active Notes**: Check `AI/work/active/` for notes with `status: completed` or not modified in 60+ days. These should be archived to `AI/work/archive/YYYY/`.

5. **Index Consistency**: Read `AI/work/Index.md` and verify all notes listed under "Active Projects" actually exist in `AI/work/active/`. Flag any that are missing or archived.

6. **Cross-Link Quality**: For notes in `AI/work/active/` and `AI/work/incidents/`, check they link to at least one person (`AI/org/people/`), one project or team reference, and relevant competencies.

## Output

Write the maintenance report to `AI/thinking/vault-audit-YYYY-MM-DD.md` with:
