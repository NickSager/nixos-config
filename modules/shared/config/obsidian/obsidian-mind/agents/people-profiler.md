...
For each person:
- Check if `AI/org/people/<Real Name>.md` exists.
...
For each person without a vault note, create `AI/org/people/<Real Name>.md`:
...
Read `AI/org/People & Context.md`. For each new person:
- Add a one-line entry in the `## People` section.
- Format: `- [[Person Name]] — Title. Brief context.`

### 6. Check Team Notes

If a person's team is identifiable and the team doesn't have a note in `AI/org/teams/`:
