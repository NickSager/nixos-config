...
- `--dry-run` — generate the migration plan without executing it (saves to `AI/thinking/`)
...
4. Check for `vault-manifest.json` in the source:
   ...
   - **Not found**: run version fingerprint detection from the target's `vault-manifest.json`:
     - Check for `claude/Memories.md` without `AI/brain/` → v1
     - Check for `AI/brain/` + `bases/` without `.claude/agents/` → v2
...
If `AI/brain/Migration Log.md` exists in the target vault, read it and extract the recorded hashes. Each log entry stores both a **source hash** (original file content) and a **final hash** (transformed content written to target).
...
|---|--------|--------|--------|------------|
| 1 | AI/org/people/Jane.md | AI/org/people/Jane.md | COPY | +description |
| 2 | AI/brain/Memories.md | AI/brain/Memories.md | REPLACE | — |
| 3 | daily/2026-03-15.md | AI/work/1-1/... | CLASSIFY+COPY | +quarter, +tags |
```
...
Migration complete.
  ...
  ✓ Indexes rebuilt (AI/work/Index.md, People & Context, Brag Doc, Memories)
  ...
  Migration log: AI/brain/Migration Log.md
```

Offer next steps:
- "Run `/om-vault-audit` for a full structural check"
- "Review `thinking/migrate-review/` for uncategorized files"
- "Check `AI/brain/Migration Log.md` for the full receipt and CLAUDE.md diff"
...
- **Non-work content is welcome.** Not every note fits work/org/perf. Personal journal entries go to `AI/reference/personal/`, book and course notes go to `AI/reference/learning/`. The goal is zero data loss, not forcing everything into work categories.
