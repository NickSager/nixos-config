...
def main():
    ...
    # Only validate markdown files in the vault's AI/ directory
    if not file_path.endswith(".md"):
        sys.exit(0)
    # Normalize path separators for cross-platform matching (Windows uses backslashes)
    normalized = file_path.replace("\\", "/")

    # Global scope guard: only validate files under ~/Documents/Notes/AI/
    # This prevents false positives when running from other project directories
    vault_ai_dir = os.path.expanduser("~/Documents/Notes/AI/")
    if vault_ai_dir not in normalized:
        sys.exit(0)
