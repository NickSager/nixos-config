...
|--------|---------------|--------|
| File in folder named `people/`, `contacts/`, `team-members/` | Person | `AI/org/people/` |
| File in folder named `teams/`, `squads/`, `groups/` | Team | `AI/org/teams/` |
| File in folder named `incidents/`, `postmortems/`, `outages/` | Incident | `AI/work/incidents/` |
| File in folder named `1-1/`, `one-on-ones/`, `meetings/` | 1:1 | `AI/work/1-1/` |
| File in folder named `archive/`, `completed/`, `done/` | Archived work | `AI/work/archive/YYYY/` |
| File in folder named `projects/`, `active/`, `wip/` | Active work | `AI/work/active/` |
| File in folder named `brag/`, `wins/`, `achievements/` | Brag | `AI/perf/brag/` |
| File in folder named `evidence/`, `pr-scans/` | Evidence | `AI/perf/evidence/` |
| File in folder named `AI/reference/`, `docs/`, `architecture/` | Reference | `AI/reference/` |
...
| Signal | Classification | Target |
|--------|---------------|--------|
| PARA: `Projects/` | Active work | `AI/work/active/` |
| PARA: `Areas/` | Ongoing responsibilities — classify by content | Varies (AI/work/active/, AI/org/, AI/brain/) |
| PARA: `Resources/` | Reference material | `AI/reference/` |
| PARA: `Archive/` | Completed or inactive | `AI/work/archive/YYYY/` |
| Zettelkasten: `Permanent/` or `Evergreen/` | Mature notes — classify by content | Varies |
| Zettelkasten: `Fleeting/` or `Scratch/` | Scratchpad | SKIP or `AI/thinking/` |
| Zettelkasten: `Literature/` or `Sources/` | Reference | `AI/reference/` |
| Daily notes: `daily/`, `journal/`, `Daily Notes/` | Journal entries — classify by content | Varies (AI/work/1-1/, thinking/) |
...
| Signal | Classification | Target |
|--------|---------------|--------|
| `<Name> YYYY-MM-DD.md` | 1:1 meeting note | `AI/work/1-1/` |
| `YYYY-MM-DD.md` (date only) | Daily note — classify by content | Varies |
| `Q[1-4] YYYY.md` in brag-like folder | Brag | `AI/perf/brag/` |
| `<Person> PRs - *.md` | PR evidence | `AI/perf/evidence/` |
...
|--------|---------------|
| `tags` contains `person` or `people` | Person → `AI/org/people/` |
| `tags` contains `team` | Team → `AI/org/teams/` |
| `tags` contains `incident` or has `severity`/`ticket` field | Incident → `AI/work/incidents/` |
| `tags` contains `work-note` or `project` and `status: active` | Active work → `AI/work/active/` |
| `tags` contains `work-note` or `project` and `status: completed`/`done` | Archived work → `AI/work/archive/YYYY/` |
| `tags` contains `competency` or `skill` | Competency → `AI/perf/competencies/` |
| `tags` contains `brain` or `north-star` or `goals` | Brain note → `AI/brain/` |
| `tags` contains `decision` or `adr` | Decision → `AI/work/active/` or `AI/work/archive/YYYY/` |
| `tags` contains `meeting` or `1-on-1` or `1:1` | 1:1 → `AI/work/1-1/` |
| `tags` contains `review` or has `cycle` field | Review artifact → `AI/perf/<cycle>/` |
| Has `person` field (not tag) pointing to a name | Evidence → `AI/perf/evidence/` |

**Inline signals (for vaults with no YAML frontmatter):**

| Signal | Classification |
|--------|---------------|
| Inline tags `#meeting`, `#1on1`, `#standup` in first 10 lines | 1:1 → `AI/work/1-1/` |
| Inline tags `#incident`, `#postmortem`, `#outage` | Incident → `AI/work/incidents/` |
| Inline tags `#project`, `#feature`, `#epic`, `#sprint` | Work note → `AI/work/active/` |
| Inline tags `#person`, `#teammate`, `#colleague` | Person → `AI/org/people/` |
| Inline tags `#decision`, `#adr` | Decision → work note |
| Inline tags `#reference`, `#architecture`, `#docs` | Reference → `AI/reference/` |
| Inline tags `#personal`, `#journal`, `#diary`, `#life` | Personal → `AI/reference/personal/` |
| Inline tags `#book`, `#reading`, `#course`, `#learning` | Learning → `AI/reference/learning/` |
...
| Signal | Classification |
|--------|---------------|
| Discusses a specific person's role, working style, or interactions | Person → `AI/org/people/` |
| Describes a project, feature, or technical work with deliverables | Work note → `AI/work/active/` or `AI/work/archive/YYYY/` |
| Contains meeting notes with attendees, discussion points, action items | 1:1 → `AI/work/1-1/` |
| References an incident, outage, or production issue with investigation | Incident → `AI/work/incidents/` |
| Contains architectural diagrams, API docs, flow descriptions | Reference → `AI/reference/` |
| Discusses goals, career plans, or focus areas | Brain → `AI/brain/` |
| Personal journal, reflections, non-work content | Personal → `AI/reference/personal/` |
| Book notes, course notes, learning material | Learning → `AI/reference/learning/` |
...
2. **Transform frontmatter**:
   ...
   - Add missing required fields per the manifest's `frontmatter_required`:
     ...
     - `status`: `active` if going to `AI/work/active/`, `completed` if going to `AI/work/archive/`
...
**Scaffold files** (AI/brain/Memories.md, AI/work/Index.md, etc.):
...
- `AI/work/Index.md`: verify work note links, add missing ones grouped by status
- `AI/brain/Memories.md`: verify topic note links, add any new AI/brain/ notes
- `AI/org/People & Context.md`: verify people and team links, add missing ones
- `AI/perf/Brag Doc.md`: verify quarter note links and evidence links

### Step 5: Write Migration Log

Write `AI/brain/Migration Log.md` with:
...
- Move `claude/` contents to `AI/brain/` (rename `claude/Memories.md` → `AI/brain/Memories.md`, etc.)
- If `claude/Memories.md` is a monolith (single file with multiple topic sections), split into separate topic files (`AI/brain/Key Decisions.md`, `AI/brain/Patterns.md`, `AI/brain/Gotchas.md`)
- Move flat `AI/work/*.md` notes into `AI/work/active/` or `AI/work/archive/YYYY/` based on status
...
- **Idempotency.** Check content hashes against `AI/brain/Migration Log.md` before processing. Skip unchanged files, flag changed ones.
